...
相较于Gemma模型，Gemma2有下面几方面的新特性：

1. **交替使用局部滑动窗口和全局注意力**：滑动窗口大小设置为4096 token，而全局注意力层的设置为8192 token。可在Google AI Studio中使用。
2. **局部-全局注意力交替和组查询注意力（GQA）**：局部注意力机制使用固定大小的滑动窗口来关注输入序列中的一部分，全局注意力机制则允许模型关注输入序列中的所有位置，每层之间交替使用两种注意力机制使得模型既能高效处理长文本，又能捕捉全局上下文信息。组查询注意力将标准的多头注意力机制中的查询头分成若干组，每组共享同样的键和值，可以减少参数数量和计算量。
3. **使用知识蒸馏技术**：通过大模型教师指导小模型训练，提高了训练效率和模型性能。蒸馏过程使用了更丰富的概率分布信息，而非传统的下一词预测任务。
4. **安全技术**：模型在训练和微调过程中进行了多层次的过滤，以避免生成有害内容。具体措施包括
   - 过滤个人信息和敏感数据
   - 使用多阶段的过滤和拒绝机制，减少错误生成和幻觉
   - 实施严格的安全策略，防止生成有害或误导性内容

> 2.6B的Gemma2模型也会尽快推出，可以部署在移动设备上
