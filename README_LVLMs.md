...
The authors argue that traditional Vision-Language Models (VLMs) are limited by their reliance on vision encoders. They provide several reasons for this limitation:

1. **Image Resolution and Aspect Ratio Constraints**: Traditional VLMs are pre-trained on fixed-size, square images. This necessitates adjustments (such as cropping, padding, or splitting) for images of different shapes, leading to distorted layouts and additional computational burden.
2. **Deployment Efficiency Bottlenecks**: In traditional VLM architectures, vision encoders and text encoders operate sequentially. As text content and image resolution increase, the deployment costs escalate significantly.
3. **Mismatch in Model Capacity and Capability**: In pre-training, Vision Models (VMs) and Language Models (LLMs) are trained separately. Disparities in data volume, data quality, and training objectives prevent their seamless integration within a single framework. Additionally, differences in parameter scales between LLMs and VMs exacerbate this mismatch (e.g., if the LLM's capacity far exceeds that of the VM, the LLM may be unable to fully utilize the rich visual features extracted by the VM, leading to performance bottlenecks).

##### Innovations

1. **Encoder-Free Architecture Design**: To address the aforementioned limitations of traditional vision-language models with vision encoders, the authors propose a unified decoder-based approach that directly integrates visual and linguistic inputs. This method avoids the constraints imposed by vision encoders on input resolution, aspect ratio, and semantic priors. Specifically, EVE introduces the following innovative approaches:
   - LLM-Centric Pre-Alignment Stage: In the initial training phase, EVE conducts vision-language pre-alignment using a frozen LLM. The goal is to enable the LLM to understand visual concepts and entities from scratch. This means that **EVE aims to position the LLM as a central pivot, striving to develop visual perception while preserving the original linguistic proficiency of the LLM**.
   - After removing the vision encoder, EVE comprehends image information through a **simplified Patch Embedding Layer (PEL)** and a **Patch Aligning Layer (PAL)**. The former divides the image into multiple patches and feeds these directly into the unified decoder. The latter employs a hierarchical aggregation strategy, comparing patch features at different levels between EVE and the pre-trained vision encoder to enhance visual perception.

2. **New Training Strategies**
   - **Initial Training Stage**: EVE performs vision-language pre-alignment using a frozen LLM to enable the LLM to understand visual concepts and entities from scratch. **During this process, EVE introduces additional visual supervision** by leveraging features from pre-trained vision encoders (e.g., CLIP) to enhance visual recognition capabilities. Specifically, EVE compares and aligns features generated by the vision encoder with text labels to achieve fine-grained visual representations.
   - **Generative Pre-Training Stage**: EVE unfreezes the patch embedding and aligning layers and continues training the entire model to better understand and process visual-linguistic content.
   - **Supervised Fine-Tuning Stage**: EVE uses multimodal dialogue and QA data for supervised fine-tuning, further enhancing the model's instruction-following capabilities and dialogue pattern learning.
